...
@Operation(
        summary = "Gets all reservations by input parameter(s).",
        description = "Gets all reservations by input parameter(s).",
        tags = "Hateoas"
)
@ApiResponses({
        ...
        @ApiResponse(responseCode = "404", description = "Restaurant and/or reservation not found by id.", content =
                {@Content(mediaType = "application/json", schema = @Schema(implementation = ErrorContainer.class))})
})
@Target({ElementType.METHOD, ElementType.ANNOTATION_TYPE})
@Retention(RetentionPolicy.RUNTIME)
@Inherited
public @interface ReservationFindAllByRestaurantId {
}